...
int main()
{
	...
	int p, q, n, phi, d, e, public_N, public_E, response_deci, response_decrypt, N1_rcv, ID1_rcv, msg2_deci, msg2_encrypt, N2_rcv, key_deci, key_encrypt, N2_deci;
	string msg1, msg2_str, N2_hex, check_N1, check_N2, key_substr, key_str;
	...
	cout << "Server listening on " << SERVER_IP << ":" << PORT << std::endl;
	...
	cout << "Connection accepted from " << SERVER_IP << endl;
	...
	while (1)
	{
		...
		send(client_socket, check_N2.c_str(), check_N2.length(), 0);
	...
	{
		key_substr = key.substr(i,4);
		key_deci = hex2dec(key_substr);
		key_encrypt = modexp(key_deci, public_E, public_N);
		key_str = decToStr(key_encrypt);
		cout << "key_substr = " << key_substr << "\n";
		cout << "key_deci = " << key_deci << "\n";
		cout << "key_encrypt = " << key_encrypt << "\n";
